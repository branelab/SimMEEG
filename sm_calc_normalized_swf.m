...
if nargout>=2
    norm_true_swf=[];
    if isfield(h.sim_data,'sig_final')
        ...
        avg_data = nan(size(h.sim_data.sens_final,1),length(h.cfg.source.vx_idx));
    end
...
if ~isempty(voxel_idx)
    ...
    % baselining
    norm_swf = bsxfun(@minus,norm_swf,nanmean(norm_swf(h.cfg.study.base_samps,:)));
