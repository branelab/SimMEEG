...
%                 f_samp = h.current_inv_tfr_freq_samp;
                ...
                v_idx = h.listbox_true_plv_contrasts.Value;
                
                for vc=1:v_idx %1:h.current_3D_plv_contrasts_seed_idx %% length(h.current_true_plv_idx)  % looping through true_data --> plv indices
                   
                    if ~isempty(true_data)
                        if  abs(true_data(v)) > str2num(h.edit_inv_plv_thresh.String)
                            cx_locs = squeeze(h.cfg.source.vx_locs(vx,:));
%                             ln_width = abs(true_data(v)*2);
                            ln_width = abs(true_data(v))/max(str2num(h.edit_inv_plv_caxis.String))*4;
                            
                            if true_data(v)>0; ln_style = '-'; elseif true_data(v)<=0; ln_style = ':'; end % ERS=red and ERD=blue
                            h.current_true_plv_plots(v) = plot3(h.axes_3D_images,cx_locs(:,1),cx_locs(:,2),cx_locs(:,3),'LineStyle',ln_style,'LineWidth',ln_width,'Color','k'); %ln_clr);
%                             if true_data(v)>0; ln_clr = 'k'; ln_style = '-'; elseif true_data(v)<=0; ln_clr = 'k'; ln_style = '-.'; end % ERS=red and ERD=blue
%                             h.current_true_plv_plots(v) = plot3(h.axes_3D_images,cx_locs(:,1),cx_locs(:,2),cx_locs(:,3),'LineStyle',ln_style,'LineWidth',ln_width,'Color',ln_clr);
