function h = sm_batch_sm_saved_monte_results(varargin)
...
for f = 1:length(fnames)
    try
        fn = fnames(f); sim_data.(fn{1}) = h.sim_data.(fn{1});
    catch
        fprintf('FieldName: %s doe not exist in h.sim_data\n',fn{1})
    end
end

%% true source
fnames = h.listbox_monte_saved_true_source.String(h.listbox_monte_saved_true_source.Value);
sim_data.cfg = rmfield(sim_data.cfg,'source'); % resetting cfg below with selected fieldnames
for f = 1:length(fnames)
    try
        fn = fnames(f); sim_data.cfg.source.(fn{1}) = h.sim_data.cfg.source.(fn{1});
    catch
        fprintf('FieldName: %s doe not exist in h.sim_data.cfg.source\n',fn{1})
    end
end
...
if isfield(sim_data.cfg.source,'TFR_results')
    ...
    for f = 1:length(fnames)
        fn = fnames(f); sim_data.cfg.source.TFR_results.(fn{1}) = h.sim_data.cfg.source.TFR_results.(fn{1});
    end
end
...
for a=1:length(h.inv_soln)
    for f = 1:length(fnames)
        try
            fn = fnames(1,f); inv_soln(a).(fn{1}) = h.inv_soln(a).(fn{1});
        catch
            fprintf('FieldName: %s doe not exist in h.inv_soln\n',fn{1})
        end
    end
end
...
if isfield(inv_soln,'soln')
    ...
    for a=1:length(h.inv_soln)
        for f = 1:length(fnames_soln)
            try fn = fnames_soln(1,f); inv_soln(a).soln.(fn{1}) = h.inv_soln(a).soln.(fn{1});
            catch
                fprintf('FieldName: %s doe not exist in h.inv_soln(a).soln\n',fn{1})
            end
        end
    end
end
...
if isfield(inv_soln,'TFR_results')
    ...
    for a=1:length(h.inv_soln)
        for f = 1:length(fnames_tfr)
            try fn = fnames_tfr(1,f); inv_soln(a).TFR_results.(fn{1}) = h.inv_soln(a).TFR_results.(fn{1});
            catch
                fprintf('FieldName: %s doe not exist in h.inv_soln(a).soln\n',fn{1})
            end
        end
