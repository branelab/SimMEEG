        ...
        switch h.inv_soln(h.current_inv_soln).Type
            case {'SPA','LCMV','sLORETA','sMCMV','bRAPBeam','TrapMUSIC'}    % BRANE Lab beamformers
                ...
                for t=1:size(h.sim_data.sens_final,3)
                    seed_swf(:,:,t) = [h.inv_soln(h.current_inv_soln).soln.wts(:,seed_idx)'*squeeze(h.sim_data.sens_final(:,:,t))']';
                    seed_swf(:,:,t) = bsxfun(@minus, seed_swf(:,:,t), nanmean(seed_swf(h.sim_data.cfg.study.base_samps,:,t)));
                    comp_swf(:,:,t) = [h.inv_soln(h.current_inv_soln).soln.wts(:,comp_idx)'*squeeze(h.sim_data.sens_final(:,:,t))']';
            ...
            case {'SIA','MIA'}    % BRANE Lab beamformers - have to use nulled_wts or plv_locs that were not in wts matrix will have nans
                ...
                for t=1:size(h.sim_data.sens_final,3)
                    ...
                    seed_swf(:,:,t) = [h.inv_soln(h.current_inv_soln).soln.nulled_wts(:,seed_idx)'*squeeze(h.sim_data.sens_final(:,:,t))']';
                    seed_swf(:,:,t) = bsxfun(@minus, seed_swf(:,:,t), nanmean(seed_swf(h.sim_data.cfg.study.base_samps,:,t)));
                    comp_swf(:,:,t) = [h.inv_soln(h.current_inv_soln).soln.nulled_wts(:,comp_idx)'*squeeze(h.sim_data.sens_final(:,:,t))']';
                    comp_swf(:,:,t) = bsxfun(@minus, comp_swf(:,:,t), nanmean(comp_swf(h.sim_data.cfg.study.base_samps,:,t)));
                    %                 seed_swf(:,:,t) = [h.inv_soln(h.current_inv_soln).soln.wts(:,seed_idx)'*squeeze(h.sim_data.sens_final(:,:,t))']';
                    %                 seed_swf(:,:,t) = bsxfun(@minus, seed_swf(:,:,t), nanmean(seed_swf(h.sim_data.cfg.study.base_samps,:,t)));
                    %                 comp_swf(:,:,t) = [h.inv_soln(h.current_inv_soln).soln.wts(:,comp_idx)'*squeeze(h.sim_data.sens_final(:,:,t))']';
            ...
            case {'eLORETA','MNE'}    % Field Trips inverse solutions
                ...
                swf=[];
                for ox=1:size(wts,3)
                    seed_swf(:,ox,:) = (squeeze(wts(:,seed_idx,ox))'*squeeze(nanmean(h.sim_data.sens_final,3))')';
                    seed_swf(:,ox,:) = bsxfun(@minus, seed_swf(:,ox,:), nanmean(seed_swf(h.sim_data.cfg.study.base_samps,ox,:)));
                    comp_swf(:,ox,:) = (squeeze(wts(:,comp_idx,ox))'*squeeze(nanmean(h.sim_data.sens_final,3))')';
