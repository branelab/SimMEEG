...
%                 display(h.current_inv_tfr_freq_point)
    elseif h.radio_inv_plot_true_tfr_connectivity.Value==1
        ...
        ss=find(h.cfg.source.TFR_results.TFR_freqs<=y);
        h.current_inv_tfr_freq_samp = ss(end);
        h.current_inv_tfr_freq_point = h.cfg.source.TFR_results.TFR_freqs(h.current_inv_tfr_freq_samp);
    end
    ...
    if h.radio_inv_plot_peak_tfr_connectivity.Value==1
        ...
        h.axes_inv_soln_tfr.Title.String = sprintf('Averaged Peak Sources: Time = %.3f Freq = %.1f Va = %.2f',h.current_inv_tfr_time_point,h.current_inv_tfr_freq_point,pt_val);
