function [htopo, hcont] = bl_ft_plot_topo3d(hax, pos, val, varargin)
...
%
% Modified by A. Herdman (UBC) May 21, 2024
  ...
  switch topostyle
    case 'color'
      ...
      if length(val)==size(pos,1)
        htopo = patch(hax, 'Vertices', pos, 'Faces', tri, 'FaceVertexCData', val, 'FaceColor', 'interp');
      else
        htopo = patch(hax, 'Vertices', pos, 'Faces', tri, 'CData', val, 'FaceColor', 'flat');
      end
      set(htopo, 'EdgeColor', 'none');
      set(htopo, 'FaceLighting', 'none');

      % if facealpha is an array with number of elements equal to the number of vertices
      if size(pos,1)==numel(facealpha)
        set(htopo, 'FaceVertexAlphaData', facealpha);
        set(htopo, 'FaceAlpha', 'interp');
      elseif ~isempty(pos) && numel(facealpha)==1 && facealpha~=1
        % the default is 1, so that does not have to be set
        set(htopo, 'FaceAlpha', facealpha);
      end
  ...
  switch contourstyle
    case 'black'
      ...
      for i=1:length(cntlevel)
        ...
        h1 = patch(hax, 'XData', X(:,i), 'Ydata', Y(:,i), ...
          'ZData', Z(:,i), 'CData', C(:,i), ...
