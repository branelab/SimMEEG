    ...
    case 'Whole Brain' % whole brain
        ...
        switch h.inv_soln(h.current_inv_soln).Type
            ...
            case {'SIA','MIA'}    % BRANE Lab beamformers
                search_dist = 15;
            case {'sMCMV','bRAPBeam','TrapMUSIC'}    % BRANE Lab beamformers
                search_dist = 15;
...
%                         h.inv_soln(h.current_inv_soln).soln.P.img = h.inv_soln(h.current_inv_soln).soln.P.residual_img;     % Rinv used for wts
                        %                         h.inv_soln(h.current_inv_soln).soln.P.img = h.inv_soln(h.current_inv_soln).soln.P.nulled_img;     % Ninv used for wts
...
if isempty(h.current_3D_peak_voxels) % no peaks found
    ...
    if h.radio_find_spatiotemp_peaks.Value == 1
        ...
        use_MCMV_flag=1; % implement if you want all inv_soln are being evaluated the same
        if isfield(h.inv_soln(h.current_inv_soln).soln,'MCMV_idx')  && use_MCMV_flag==1 % if SIA or MIA, first find closest sources using found MCMV idx then use nulled image peaks to find nearest sources
            MCMV_idx = h.inv_soln(h.current_inv_soln).soln.MCMV_idx;
            x_idx = ismember(MCMV_idx,h.current_3D_peak_idx);
            MCMV_idx = MCMV_idx(x_idx); % selecting only indices that are above threshold
            p_locs = h.inv_soln(h.current_inv_soln).leadfield.voxel_pos(MCMV_idx,:);
            diff_locs=[]; min_idx=zeros(1,3); peak_idx=zeros(1,3);
            if ~isempty(p_locs)
                [v_idx]=find_nearest_voxel(p_locs,vx_locs);    % find nearest true_source for each peak source
            else
                v_idx=[];
            end
            if length(unique(v_idx))<3 && ~isempty(unique(v_idx))
                vv_idx = unique(v_idx,'stable');
                for vx=vv_idx; diff_locs(vx,:) = ( nanmean( (p_locs-vx_locs(vx,:)).^2,2)).^.5; end
                for vx=vv_idx; vvx=find(v_idx==vx); [mx_val,mx] = min( diff_locs(vx,vvx) ); min_idx(vx)=vvx(mx); end
                peak_idx(min_idx~=0) = MCMV_idx(min_idx(min_idx~=0));
                % finding missed true source within residual map
                missed_idx = find(peak_idx==0);
                leftover_idx = setxor(h.current_3D_peak_voxels(:,5),MCMV_idx);  % removing already found MCMV peaks
                p_locs = h.inv_soln(h.current_inv_soln).leadfield.voxel_pos(leftover_idx,:);
                [v_idx]=find_nearest_voxel(p_locs,vx_locs);    % find nearest true_source for each peak source
                
                if ismember(missed_idx,v_idx)  % if there are nearby other peaks within the map at this threshold level
                    diff_locs=[]; min_idx=zeros(1,3);
                    for vx=missed_idx; diff_locs(vx,:) = ( nanmean( (p_locs-vx_locs(vx,:)).^2,2)).^.5; end
                    for vx=missed_idx; vvx=find(v_idx==vx); [mx_val,mx] = min( diff_locs(vx,vvx) ); min_idx(vx)=vvx(mx); end
                    peak_idx(missed_idx) = leftover_idx(min_idx(min_idx~=0));
                else % no other peaks within the map at this threshold level
                    [v_idx]=find_nearest_voxel(vx_locs(missed_idx,:),h.inv_soln(h.current_inv_soln).leadfield.voxel_pos(MCMV_idx,:));    % find nearest true_source for each peak source
                    peak_idx(missed_idx) = MCMV_idx(v_idx);
                end
            elseif length(unique(v_idx))==3 && ~isempty(unique(v_idx))
                for vx=v_idx; diff_locs(vx,:) = ( nanmean( (p_locs-vx_locs(vx,:)).^2,2)).^.5; end
                for vx=v_idx; vvx=find(v_idx==vx); [mx_val,mx] = min( diff_locs(vx,vvx) ); min_idx(vx)=vvx(mx); end
                peak_idx = MCMV_idx(min_idx);
            else
                peak_idx=[];    % no sources found
            end
            
            if ~isempty(peak_idx)
                diff_idx = setxor(h.current_3D_peak_voxels(:,5),peak_idx);
                try v_idx = [peak_idx diff_idx']; catch; v_idx = [peak_idx diff_idx]; end
                ...
                h.current_3D_peak_idx = []; % MCMV_idx found and re-ordered to be nearest to true sources
                h.current_3D_peak_voxels = [];  % re-ordered
                sm_calc_localizer_performance;
            end
