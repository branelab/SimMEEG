function h = sm_batch_run_source_modeling(h)
%% Run Source modeling
...
switch h.menu_inv_soln.String{h.menu_inv_soln.Value}
    ...
    case {'SIA' 'MIA' 'sMCMV' 'bRAPBeam' 'TrapMUSIC'}      % multi-source scalar beamformers seee Herdman et al., 2018 Brain Topography
...
switch h.menu_inv_soln.String{h.menu_inv_soln.Value}
    ...
    %% Brainstorm Parameters are gathered below when running inverse solution
...
%         data = permute(repmat(x.avg_data.F,[1 1 90]),[2 1 3]);
        h = sm_batch_sm_bst_noisecov(h,data);  % create Noise and Data covariances from Brainstorm function (changed by ATH only for entering data - calculations are those from Brainstorm)
...
h.slider_3D_image_thresh.Min = min_max(1); 
h.slider_3D_image_thresh.Max = min_max(2); 

h = sm_batch_update_image_thresh_txt(h);
...
h = sm_batch_set_3D_image_thresh(h); % re-thresholding data based on null_thresh
