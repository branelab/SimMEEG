...
if ~isfield(h.cfg.source,'TFR_results')
    ...
    if isempty(h.cfg.source.TFR_results) && h.radio_inv_plot_true_tfr_connectivity.Value ==1
        msgbox(sprintf('Please perform Time-Frequency Response (TFR) analyses\n\nClick on "Calc True Connectivity"\n'));
        h.radio_inv_plot_true_tfr_connectivity.Value = 0;
        ...
        if h.radio_inv_plot_true_tfr_connectivity.Value == 0
            ...
            % find if v_idx is/are in true_idx or comp_idx
            
            true_idx = ismember(1:3,v_idx);
            ...
            switch h.menu_inv_tfr_type.String{h.menu_inv_tfr_type.Value}
                case 'Total Power'
                    switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                        case 'Data'; tfr_data = h.cfg.source.TFR_results.avg_true_wt(:,:,true_idx); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Noise'; tfr_data = h.cfg.source.TFR_results.Noise.avg_true_wt(:,:,true_idx); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                    end
                case 'Evoked Power'
                    switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                        case 'Data'; tfr_data = h.cfg.source.TFR_results.avg_true_wt_evk(:,:,true_idx); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Noise'; tfr_data = h.cfg.source.TFR_results.Noise.avg_true_wt_evk(:,:,true_idx); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                    end
                case 'Induced Power'
                    switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                        case 'Data'; tfr_data = h.cfg.source.TFR_results.avg_true_wt_ind(:,:,true_idx); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Noise'; tfr_data = h.cfg.source.TFR_results.Noise.avg_true_wt_ind(:,:,true_idx); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                    end
                case 'PLV'
                    switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                        case 'Data'; tfr_data = permute(h.cfg.source.TFR_results.plv_based(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Noise'; tfr_data = permute(h.cfg.source.TFR_results.Noise.plv_based(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Surrogate'
                            if ~isempty(h.cfg.source.TFR_results.plv_surg_based_mean)
                                tfr_data = permute(h.cfg.source.TFR_results.plv_surg_based_mean(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                            else; tfr_data = [];
                            end
                        case 'Data (Surrogate thresholded)'
                            tfr_data = permute(h.cfg.source.TFR_results.plv_based(:,:,:),[1 3 2]);
                            mu = permute(h.cfg.source.TFR_results.plv_surg_based_mean(:,:,:),[1 3 2]);
                            sigma = permute(h.cfg.source.TFR_results.plv_surg_based_std(:,:,:),[1 3 2]);
                        ...
                        case 'Data (Noise thresholded)'
                            tfr_data = permute(h.cfg.source.TFR_results.plv_based(:,:,:),[1 3 2]);
                            mu = permute(h.cfg.source.TFR_results.Noise.plv_based(:,:,:),[1 3 2]);
                            sigma = repmat( nanstd(permute(h.cfg.source.TFR_results.Noise.plv_based(:,:,:),[1 3 2]),[],2) ,[1 size(mu,2) 1]);
                ...
                case 'PLI'
                    switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                        case 'Data'; tfr_data = permute(h.cfg.source.TFR_results.pli_based(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Noise'; tfr_data = permute(h.cfg.source.TFR_results.Noise.pli_based(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Surrogate'; tfr_data = permute(h.cfg.source.TFR_results.pli_surg_based_mean(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Data (Surrogate thresholded)'
                            tfr_data = permute(h.cfg.source.TFR_results.pli_based(:,:,:),[1 3 2]);
                            mu = permute(h.cfg.source.TFR_results.pli_surg_based_mean(:,:,:),[1 3 2]);
                            sigma = permute(h.cfg.source.TFR_results.pli_surg_based_std(:,:,:),[1 3 2]);
                            x_std =tinv(1-h.FC_alpha_level,2); % using df = 2 because contrast is between data and surrogate
                            surg_thresh_pos = mu + (x_std*sigma);   % upper confidence interval
                            surg_thresh_neg = mu - (x_std*sigma);   % lower confidence interval
                        ...
                        case 'Data (Noise thresholded)'
                            tfr_data = permute(h.cfg.source.TFR_results.pli_based(:,:,:),[1 3 2]);
                            mu = permute(h.cfg.source.TFR_results.Noise.pli_based(:,:,:),[1 3 2]);
                            sigma = repmat( nanstd(permute(h.cfg.source.TFR_results.Noise.pli_based(:,:,:),[1 3 2]),[],2) ,[1 size(mu,2) 1]);
                            x_std =tinv(1-h.FC_alpha_level,2); % using df = 2 because contrast is between data and noise
                            surg_thresh_pos = mu + (x_std*sigma);   % upper confidence interval
                            surg_thresh_neg = mu - (x_std*sigma);   % lower confidence interval
                            % creating significance mask
                    ...
                    lat = h.cfg.source.TFR_results.pli_lat;
                case 'dPLI'
                    switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                        case 'Data'; tfr_data = permute(h.cfg.source.TFR_results.dpli_based(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Noise';  tfr_data = permute(h.cfg.source.TFR_results.Noise.dpli_based(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Surrogate'; tfr_data = permute(h.cfg.source.TFR_results.dpli_surg_based_mean(:,plv_true,:),[1 3 2]); h.current_inv_true_fc_data = permute(tfr_data,[1 3 2]);
                        case 'Data (Surrogate thresholded)'
                            tfr_data = permute(h.cfg.source.TFR_results.dpli_based(:,:,:),[1 3 2]);
                            mu = permute(h.cfg.source.TFR_results.dpli_surg_based_mean(:,:,:),[1 3 2]);
                            sigma = permute(h.cfg.source.TFR_results.dpli_surg_based_std(:,:,:),[1 3 2]);
                            x_std =tinv(1-h.FC_alpha_level,2); % using df = 2 because contrast is between data and surrogate
                            surg_thresh_pos = mu + (x_std*sigma);   % upper confidence interval
                            surg_thresh_neg = mu - (x_std*sigma);   % lower confidence interval
                        ...
                        case 'Data (Noise thresholded)'
                            tfr_data = permute(h.cfg.source.TFR_results.dpli_based(:,:,:),[1 3 2]);
                            mu = permute(h.cfg.source.TFR_results.Noise.dpli_based(:,:,:),[1 3 2]);
                            sigma = repmat( nanstd(permute(h.cfg.source.TFR_results.Noise.dpli_based(:,:,:),[1 3 2]),[],2) ,[1 size(mu,2) 1]);
                            x_std =tinv(1-h.FC_alpha_level,2); % using df = 2 because contrast is between data and noise
                            surg_thresh_pos = mu + (x_std*sigma);   % upper confidence interval
                            surg_thresh_neg = mu - (x_std*sigma);   % lower confidence interval
                            % creating significance mask
            ...
            tfr_freqs = h.cfg.source.TFR_results.TFR_freqs;
            ...
            if ~isempty(tfr_data)
                ...
                h.colorbar_axes_true_tfr = colorbar('Location','eastoutside');
                
                plot3(h.axes_true_source_tfr,lat_coi,h.cfg.source.TFR_results.coi_wt2,ones(size(h.cfg.source.TFR_results.coi_wt2))*tfr_caxis(2),'color',[1 1 1]*.7,'linewidth',2);
