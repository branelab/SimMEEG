function sm_bst_noisecov(varargin)
...
% Not sure if Brainstorm used average data for Cov calculations or trial data. Online tutorials state that Brainstorm use trial data so going with that one
for t=1:size(data,3) % across trials
    
%     DataMat.F = squeeze(nanmean(data,3))';   % struct = [samps x sens]; Brainstorm uses averaged data for Cov calculations.
    DataMat.F = squeeze(data(:,:,t))';   % struct = [samps x sens]; Brainstorm uses averaged data for Cov calculations.
    
    DataMat.Leff = size(data,3);    % number of trials averaged
    DataMat.nAvg = size(data,3); % number of trials averaged
...
h.inv_soln(h.current_inv_soln).soln.bst_cov.DataCovMat.nSamples = Ntotal;
...
h.inv_soln(h.current_inv_soln).soln.bst_cov.NoiseCovMat.Comment = 'Noise covariance';
h.inv_soln(h.current_inv_soln).soln.bst_cov.NoiseCovMat.nSamples = Ntotal;
