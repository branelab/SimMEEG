...
%                 f_samp = h.current_inv_tfr_freq_samp;
                ...
                switch h.menu_inv_tfr_type.String{h.menu_inv_tfr_type.Value}
                    case 'PLV'
                        switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                            case 'Data'  
                                if ~isempty(h.cfg.source.TFR_results.plv_based); true_data = squeeze(h.cfg.source.TFR_results.plv_based(f_samp,true_idx,t_samp)); else; true_data = []; end 
                            case 'Noise'; true_data = [];
                            case 'Surrogate'
                                if ~isempty(h.cfg.source.TFR_results.plv_surg_based_mean); true_data = squeeze(h.cfg.source.TFR_results.plv_surg_based_mean(f_samp,true_idx,t_samp)); else; true_data = []; end
                    ...
                    case 'PLI'
                        ...
                        switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                            case 'Data';  true_data = squeeze(h.cfg.source.TFR_results.pli_based(f_samp,true_idx,t_samp));
                            case 'Noise'; true_data = [];
                            case 'Surrogate';  true_data = squeeze(h.cfg.source.TFR_results.pli_surg_based_mean(f_samp,true_idx,t_samp));
                    ...
                    case 'dPLI'
                        ss = find(h.cfg.source.TFR_results.pli_lat<h.current_inv_tfr_time_point); t_samp=ss(end);
                        switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                            case 'Data';  true_data = squeeze(h.cfg.source.TFR_results.dpli_based(f_samp,true_idx,t_samp));
                            case 'Noise'; true_data = [];
                            case 'Surrogate';  true_data = squeeze(h.cfg.source.TFR_results.dpli_surg_based_mean(f_samp,true_idx,t_samp));
                            case 'Data (Surrogate thresholded)'; true_data = h.current_inv_true_fc_data(f_samp,true_idx,t_samp);
                            case 'Data (Noise thresholded)'; true_data = h.current_inv_true_fc_data(f_samp,true_idx,t_samp);
                        end
                    case {'Total Power' 'Evoked Power' 'Induced Power'}
                        % defaulting to PLV
                        switch h.menu_inv_tfr_data_type.String{h.menu_inv_tfr_data_type.Value}
                            case 'Data';  true_data = squeeze(h.cfg.source.TFR_results.plv_based(f_samp,true_idx,t_samp));
                            case 'Noise'; true_data = [];
                            case 'Surrogate';  true_data = squeeze(h.cfg.source.TFR_results.plv_surg_based_mean(f_samp,true_idx,t_samp));
                            case 'Data (Surrogate thresholded)'; true_data = h.current_inv_true_fc_data(f_samp,true_idx,t_samp);
                            case 'Data (Noise thresholded)'; true_data = h.current_inv_true_fc_data(f_samp,true_idx,t_samp);
                        end
                end
